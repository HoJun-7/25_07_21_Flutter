...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // final screenHeight = MediaQuery.of(context).size.height; // 현재는 사용되지 않지만 필요에 따라 활용 가능

    // 로그인 카드(흰색 배경 컨테이너)의 최대 너비를 정의합니다.
    // 이는 큰 화면에서 카드가 너무 넓어지는 것을 방지합니다.
    final double maxCardWidth = 600;

    // 로고 이미지의 크기를 반응형으로 설정합니다.
    // 화면 너비의 25%를 기준으로 하되, 최소 120, 최대 200으로 제한합니다.
    final double imageSize = screenWidth * 0.25;
    final double clampedImageSize = imageSize.clamp(120.0, 200.0);
      ...
      child: Scaffold(
        ...
        body: Center(
          child: SingleChildScrollView(
            // 화면 너비의 5%를 좌우 여백으로 설정하여 반응성을 높입니다.
            padding: EdgeInsets.symmetric(horizontal: screenWidth * 0.05),
            child: Align( // SingleChildScrollView의 자식 위젯의 너비를 제한하기 위해 Align을 사용합니다.
              alignment: Alignment.topCenter, // 컨텐츠를 상단 중앙으로 정렬
              child: Container(
                constraints: BoxConstraints(maxWidth: maxCardWidth), // 로그인 카드의 최대 너비를 제한합니다.
                ...
                child: Column(
                  mainAxisSize: MainAxisSize.min, // Column이 필요한 최소한의 공간만 차지하도록 합니다.
                  children: [
                    Image.asset(
                      'assets/icon/cdss-icon_500.png',
                      width: clampedImageSize, // 반응형으로 계산된 이미지 크기 적용
                      height: clampedImageSize, // 반응형으로 계산된 이미지 크기 적용
                    ),
