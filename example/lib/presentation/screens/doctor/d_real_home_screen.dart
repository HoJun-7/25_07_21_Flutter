...
class _DRealHomeScreenState extends State<DRealHomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // ✅ 상단바 날짜/위치/기온 API 초기화 (3번 반영된 ViewModel 기준)
      vm.init();
    });
  }
...
/// ===================== 상단 우측 상태 박스 (날짜·위치·기온) =====================
class _TopRightStatusBox extends StatelessWidget {
  const _TopRightStatusBox({super.key});
  ...
  Widget build(BuildContext context) {
    return Consumer<DoctorDashboardViewModel>(
      builder: (context, vm, _) {
        ...
        return InkWell(
          onTap: () => vm.refreshWeather(), // 탭하면 날씨 갱신
          borderRadius: BorderRadius.circular(12),
          child: Container(
            height: 80,
            width: 230,
            ...
            child: vm.isLoadingWeather
                ...
                : Column(
                    ...
                    children: [
                      Text(
                        (vm.timeText.isEmpty) ? '로딩 중…' : vm.timeText,
                        maxLines: 1,
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  w?.locationLabel ?? '위치 확인 중',
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                  style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600),
                                ),
                                Text(
                                  vm.weatherError != null
                                      ? '갱신 실패'
                                      : (w?.description ?? '날씨 정보 없음'),
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
