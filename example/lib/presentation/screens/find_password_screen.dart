...
// 비밀번호 찾기 화면을 담당하는 StatelessWidget
class FindPasswordScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // FindPasswordViewModel을 Provider를 통해 제공
    return ChangeNotifierProvider(
      create: (_) => FindPasswordViewModel(baseUrl: baseUrl),
      child: const _FindPasswordForm(), // 실제 폼 위젯
    );
  }
}

// 비밀번호 찾기 폼을 구성하는 내부 위젯
class _FindPasswordForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 이름과 전화번호 입력을 위한 TextEditingController
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (viewModel.isLoading)
                    ...
                    _GradientButton( // 정의된 _GradientButton 위젯 사용
                  ...
                  AnimatedSwitcher(
                    ...
                    child: (viewModel.successMessage != null)
                        ? Text(
                            key: const ValueKey('successMessage'), // AnimatedSwitcher를 위한 고유 키
                            viewModel.successMessage!,
                            textAlign: TextAlign.center, // 중앙 정렬
                            style: const TextStyle(
                              color: Color(0xFF70FF70), // 더 눈에 띄는 초록색 계열
                              fontWeight: FontWeight.bold,
...
class _GradientButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      // 버튼 컨테이너에 그림자 및 모서리 둥글게 적용
      decoration: BoxDecoration(
        gradient: gradient, // 전달받은 그라데이션 적용
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(30.0), // InkWell의 둥근 모서리
          child: Center(
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 15.0, horizontal: 20.0), // 버튼 내부 패딩
              child: child, // 버튼 텍스트 또는 다른 위젯
            ),
          ),
        ),
      ),
